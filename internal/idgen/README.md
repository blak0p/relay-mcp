...
This is a flat package (no sub-packages) because it owns a single, small
responsibility. See `internal/session/README.md` for how ids flow into the
session lifecycle.
...
`math/rand` is seeded for pseudo-randomness and is the wrong tool for
identifiers that must not collide under an adversarial model. `crypto/rand`
reads from the kernel's CSPRNG (`getrandom(2)` on Linux), which is
non-blocking on Linux 5.6+ and requires no seeding. The cost is negligible
for a single-session server that generates one id per session.

## Collision analysis

The id space is 2^64 (8 random bytes). Under the birthday paradox, you would
need ~2^32 sessions (~4 billion) for a 50% collision chance. relay-mcp serves
one session at a time over stdio, so the practical collision probability is
effectively zero.
